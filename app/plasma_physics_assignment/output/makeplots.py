...
nonlost = np.abs(np.loadtxt('nonlost_velocity.csv', delimiter=';'))
lost = np.abs(np.loadtxt('lost_velocity.csv', delimiter=';'))
plt.figure()
pnonlost, = plt.plot(nonlost[:, 0], nonlost[:, 1], 'bx', label='nonlost')
plost, = plt.plot(lost[:, 0], lost[:, 1], 'rx', label='lost')
plt.xlabel('radial velocity (m/s)')
plt.ylabel('transverse velocity (m/s)')
plt.legend(handles=[pnonlost, plost])
